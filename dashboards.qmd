...
description: "To assess how CCRS data can address (or fails to address) concerns raised by the Washington State Auditor in October 2024"
format:
  html:
    theme: flatly
    toc: true
...
Below are interactive dashboards exploring CCRS data addressing (or fails to address) concerns raised by the Washington State Auditor in an October 2024 agency press release, in real time.  The data covers Dec 2021 to today.  
...
We track quality, outliers, non-conformers, and more to address total traceability.  Everyone has concerns about regulated weed crossing the border, and tracking the above KPIs can point regulators to license holders to validate entries.   
```{r setup}
#knitr::opts_chunk$set(echo = FALSE, include=FALSE, message=FALSE, error=FALSE)
...
## Number of Licensees (2021-present)
```{r, echo=TRUE}
...
We include illustrative estimates for 2023–2025 based on market maturity and CCRS data. 
...
**Partial Traceability – Example Joins from Plant → Inventory → Sale**
```{r, echo=TRUE}
...
inventory_lag_filtered <- inventory_lag %>% filter(LagDays >= 0)

ggplot(inventory_lag_filtered, aes(x = LagDays)) +
  geom_histogram(binwidth = 5, fill = "steelblue") +
  labs(title = "Distribution of Inventory Lag (positive values view)",
...
**Summary of Lab Result Coverage by Product Type**  

Let’s consider:  

+ **Fail** and **Required** as **problematic/non-compliant**  
+ **Pass** as **compliant**
```{r, echo=TRUE}

lab_summary <- dbGetQuery(con, "
  SELECT 
    p.InventoryType,
    COUNT(l.LabResultId) AS TotalTests,
    SUM(CASE 
          WHEN LOWER(TRIM(l.LabTestStatus)) IN ('fail', 'required') 
          THEN 1 ELSE 0 
        END) AS ProblemTests,
    ROUND(
      100.0 * SUM(CASE 
                   WHEN LOWER(TRIM(l.LabTestStatus)) IN ('fail', 'required') 
                   THEN 1 ELSE 0 
                 END) / COUNT(l.LabResultId), 1
    ) AS PctProblem
  FROM lab_result_all l
  LEFT JOIN inventory_all i ON l.InventoryId = i.InventoryId
  LEFT JOIN product_all p ON i.ProductId = p.ProductId
  GROUP BY p.InventoryType
  ORDER BY PctProblem DESC
")
...
lab_summary %>%
  kbl(caption = "Lab Test Summary by Inventory Type") %>%
  kable_styling(full_width = FALSE)
