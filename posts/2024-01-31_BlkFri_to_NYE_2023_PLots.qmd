---
title: "Black Friday 2023 to New Year’s Day in Washington’s CCRS Era"
...
filter(total_revenue > 200) %>%
arrange(desc(total_revenue)) %>%
mutate(total_revenue = dollar(total_revenue)) %>%
rename('Total Revenue' = total_revenue) %>%
kable(caption = "Total Revenue by Product Type (Black Friday to New Year's Day)") %>%
...
holiday_sales %>%
  ...
  scale_y_log10(labels = dollar_format(), breaks = c(100, 1000, 10000, 100000, 1000000, 10000000), limits = c(100, 10000000)) +
  labs(
...
kable(caption = "Average Discount % by Product Type (Discount > 5.0%)") %>%
kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
kable(caption = "Total Revenue per Licensee – Holiday Season 2023 (Revenue > $10,000) ") %>%
kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
+ Concentrate Markets Matured: Solvent and solventless producers increased competition — reflected in markdown levels approaching 25%.
