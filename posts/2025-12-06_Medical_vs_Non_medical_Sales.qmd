...
With over 27 million transaction records analyzed through November 2025, the Evergreen Canna Ledger examined how revenue, discounting, and taxation vary between the two consumer groups.
...
sales_data <- sales_data %>% distinct(.keep_all = TRUE)
...
```{r, include=TRUE, fig.width=40, fig.height=25, warning=FALSE, message=FALSE}

plot_revenue_marked <- 
  ggplot(plot_revenue_marked %>% filter(!is.na(Category)), aes(x = WeekStart, y = WeeklyRevenue, color = Category)) +
