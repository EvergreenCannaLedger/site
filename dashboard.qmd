...
WHERE DATE(CAST(sh.SaleDate AS DATE)) BETWEEN '2021-12-01' AND '2026-01-02'
...
layout(
title = "Number of Licensees Reporting Monthly (Dec 2021 - Jan 2026)",
...
product_growth <- product_growth %>% mutate(Month = as.Date(Month)) %>% distinct(.keep_all = TRUE) 
...
plot_ly(retail_wholesale, x = ~Month, y = ~Revenue, color = ~SaleType, type = "bar") %>%
  layout(
    barmode = "group",
    title = "Total Reported Wholesale Sales Revenue",
    xaxis = list(title = "Month"),
...
regional_sales <- dbGetQuery(con, "
  ...
  FROM (
    ...
    WHERE sh.SaleDate IS NOT NULL
      ...
      AND TRY_CAST(sd.Quantity AS DOUBLE) IS NOT NULL
      AND TRY_CAST(sd.UnitPrice AS DOUBLE) IS NOT NULL
  ) AS distinct_sales
  GROUP BY City
...
layout(
  title = "Top 10 Business Licensee Cities by Reported Revenue",
  xaxis = list(title = "City"),
  yaxis = list(
    title = "Revenue",
    tickformat = "$,"  # Dollar formatting
  ),
  margin = list(b = 100)
)

```
## Product Category Trends (2021–2025)
```{r, warning=FALSE}
...
category_summary %>%
  ...
  datatable(
    ...
    filter = "top",          # 👈 adds column filters
    rownames = FALSE,
    options = list(
      pageLength = 20,       # 👈 show 20 rows at a time
